...
$q="select * from NETFLOWENTRY";                         //test 테이블의 레코드를 모두 뽑아오기
...
<td>$dbUNIX_SECS[$i]</td><td>$dbSRC_ADDR[$i]</td><td>".long2ip($dbDST_ADDR[$i])."</td><td>$dbISTCP[$i]</td><td>$dbPACKET_COUNT[$i]</td>
<td>$dbBYTE_COUNT[$i]</td><td>".date("M j G:i:s Y", $dbSTART_TIME[$i])."</td><td>$dbEND_TIME[$i]</td><td>$dbSRC_PORT[$i]</td><td>$dbDST_PORT[$i]</td>
